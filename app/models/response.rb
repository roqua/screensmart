...
class Response < BaseModel
  ...
  %i(domain_results next_question_id done).each do |r_attribute|
  ...
  def results
    @results ||= domain_ids.map do |domain_id|
      DomainResult.new({ domain_id: domain_id }.merge(domain_results[domain_id]))
    end
  end
